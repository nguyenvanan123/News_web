...
public class PostsDAO implements IPostsDAO {
    ...
    private static final String SELECT_USER_BY_ID = "select  title,  content , timestamp, avatar, shortdescription, access from createposts where id =?";
    ...
    private static final String FILTER_POST_WITH_TITLE_SQL = "select * from createposts where title like ?";
    public PostsDAO() {
    }
    ...
    @Override
    public List<Posts> filterPosts(String titleFilter) throws SQLException {
        List<Posts> posts = new ArrayList<>();
        try (Connection connection = getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(FILTER_POST_WITH_TITLE_SQL)) {
            ...
            while (rs.next()) {
                ...
                String avatar = rs.getString("avatar");
                String shortdescription = rs.getString("shortdescription");
                int numberofcomment = rs.getInt("numberofcomment");
                int numberoflike = rs.getInt("numberoflike");
                boolean access = rs.getBoolean("access");
                posts.add(new Posts(id, title, content, timestamp, avatar, shortdescription, numberofcomment, numberoflike, access));
            }
